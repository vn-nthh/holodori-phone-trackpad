...
One-time setup:
  1. Give your user access to /dev/uinput:
     ...
     Log out and back in for the group change to take effect. If your
     distribution does not create an `input` group, install a udev rule
     instead, for example a file at
     /etc/udev/rules.d/99-uinput.rules containing:
  ...
  2. Allow inbound UDP 42825 on the USB-tethered interface. Example with
     ...
     On firewalld:
       sudo firewall-cmd --zone=<tether-zone> --add-port=42825/udp
...
Start the Linux app:
  ...
  4. "Stop the PC from using the phone's internet" is Windows-only; the
     checkbox is disabled in this build. USB tethering's route normally
     loses to your real uplink on Linux on its own (interface metric ranks
     it below a wired/Wi-Fi uplink). If you still need to guarantee it,
     configure NetworkManager directly instead of running a privileged
     process that rewrites the routing table:
     ...
     tethered link from the first command's output. An experimental native
     implementation of the old per-command elevation approach still exists
     behind the linux-tether-policy Cargo feature in native-host; it is not
     built by default and is not supported.
  5. Press Start.
