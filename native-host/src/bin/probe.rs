// This is a Windows Touch diagnostic: it opens a Win32 window and reports
// WM_POINTER messages against it. There is no Linux equivalent (touch mode
// is Windows-only), so the binary still builds and runs elsewhere, but only
// to say so.
...
mod imp {
    ...
    use windows_sys::Win32::Graphics::Gdi::{
        ...
        GetStockObject, InvalidateRect, NULL_PEN, PAINTSTRUCT, ScreenToClient, SelectObject,
...
#[cfg(not(windows))]
fn main() {
    eprintln!("holodori-touch-probe is Windows-only");
    std::process::exit(1);
}
