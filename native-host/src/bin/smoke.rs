// Drives the Windows Touch sink end to end against the probe window. There
// is no Linux equivalent (touch mode is Windows-only), so the binary still
// builds and runs elsewhere, but only to say so.
...
#[cfg(not(windows))]
fn main() {
    eprintln!("holodori-touch-smoke is Windows-only");
    std::process::exit(1);
}
