...
pub fn install_shutdown_handler(
    ...
    static REQUESTED: OnceLock<&'static AtomicBool> = OnceLock::new();
    REQUESTED.set(requested).ok();

    extern "C" fn handle_signal(_signal: libc::c_int) {
        // Only an atomic store: this runs as a signal handler, and nothing
        // else is guaranteed async-signal-safe.
        if let Some(requested) = REQUESTED.get() {
            requested.store(true, Ordering::Relaxed);
        }
    }
...
pub fn raise_input_priority() {
    ...
    //
    // `setpriority` returns -1 both for a genuine error and legitimately as
    // the successfully-set nice value when that value is -1, so the return
    // value alone cannot distinguish success from failure; errno must be
    // checked (cleared first, since a prior call elsewhere may have left a
    // stale one). A negative nice value needs `CAP_SYS_NICE` or a raised
    // `RLIMIT_NICE`, neither of which an unprivileged user has on stock
    // distros, so `EACCES`/`EPERM` here is the expected, common outcome for
    // most Linux users, not a problem to report on every single launch. Any
    // other errno is unexpected and still worth a warning.
