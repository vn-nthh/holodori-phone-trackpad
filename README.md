...
Lane keys go through the kernel's `uinput` virtual keyboard. On a stock
system `/dev/uinput` is owned `root:root` with mode `0600`. The `input`
group exists on essentially every systemd distribution, but nothing ships a
rule that grants that group access to `/dev/uinput` by default, so adding
yourself to the group alone does nothing until something also grants the
group access to the device.

1. Install a udev rule that grants the `input` group access to
   `/dev/uinput`. Create `/etc/udev/rules.d/99-uinput.rules` containing:
...
2. Reload udev and re-trigger it so the rule takes effect without a reboot:
...
4. Log out and back in. A group change does not apply to an already-open
   session.
...
5. Verify it worked:
   ...
   The group should be `input` with group read and write, for example
...
USB tethering's route normally loses the default-route race to your real
uplink on Linux on its own: NetworkManager (or your distribution's network
manager) ranks routes by interface metric, and a wired or Wi-Fi uplink
typically outranks a tethered USB link already. If you still need to
guarantee the PC never routes through the phone, configure NetworkManager
directly instead of running a privileged process that rewrites the routing
table:

```sh
nmcli connection show
nmcli connection modify <tether-connection-name> ipv4.never-default yes
...
An experimental native implementation of the old per-command elevation
approach still exists behind the `linux-tether-policy` Cargo feature in
`native-host`. It is not built by default and is not supported.
...
should show group `input` with group read and write (`crw-rw----`). If it
...
of the `input` group (`groups "$USER"`) and that you logged out and back in
after joining it.
